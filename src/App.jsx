...
export default function App() {
  ...
  const weekData   =useMemo(()=>weekDays().map(d=>({date:d,xp:comps.filter(c=>c.date===d).reduce((s,c)=>s+c.earnedXp,0),list:comps.filter(c=>c.date===d)})),[comps]);
  const monArr     =useMemo(()=>monDays(),[]);
  const xpDay      =useMemo(()=>{const m={};comps.forEach(c=>{m[c.date]=(m[c.date]||0)+c.earnedXp});return m;},[comps]);
  const monXP      =useMemo(()=>comps.filter(c=>c.date.startsWith(today.slice(0,7))).reduce((s,c)=>s+c.earnedXp,0),[comps,today]);
  const monCnt     =useMemo(()=>comps.filter(c=>c.date.startsWith(today.slice(0,7))).length,[comps,today]);
  const actDays    =useMemo(()=>new Set(comps.filter(c=>c.date.startsWith(today.slice(0,7))).map(c=>c.date)).size,[comps,today]);
