...
// Storage — localStorage with optional window.storage fallback (Claude artifacts)
function sGet(key) {
...
function getWeekStart() {
  ...
  const mon = new Date(t); mon.setDate(t.getDate()-offset); return localDate(mon);
}
...
function getWeekDays(){
  const t=new Date(),offset=(t.getDay()+6)%7;
  return Array.from({length:7},(_,i)=>{const d=new Date(t);d.setDate(t.getDate()-offset+i);return localDate(d);});
}
...
export default function App() {
  const [templates,  setTemplates]  = useState(()=>migrateTemplates(sGet("sl_tpl")??INIT_TEMPLATES));
  const [completions,setCompletions]= useState(()=>sGet("sl_comp")??[]);
  const [player,     setPlayer]     = useState(()=>sGet("sl_plr")??{name:"Tim",streak:0,lastDate:null});
  const [tab,        setTab]        = useState("today");
  const [flash,      setFlash]      = useState(null);
  const [showAdd,    setShowAdd]    = useState(false);
  const [newQ,       setNewQ]       = useState({name:"",category:"sonstige",difficulty:"normal",emoji:"📋",frequency:"daily"});

  useEffect(()=>{ sSet("sl_tpl",templates);   },[templates]);
  useEffect(()=>{ sSet("sl_comp",completions); },[completions]);
  useEffect(()=>{ sSet("sl_plr",player);       },[player]);
  ...
  function doComplete(t){
    ...
    setPlayer(p=>{const yest=localDate(new Date(Date.now()-86400000));let s=p.streak;if(p.lastDate!==today){s=(p.lastDate===yest||!p.lastDate)?s+1:1;}return{...p,streak:s,lastDate:today}});
  ...
  function doUndo(templateId,isWeekly=false){
    setCompletions(p=>{
      ...
      const lastId=pool.at(-1).id;return p.filter(c=>c.id!==lastId);
    });
  }
  function doAdd(){
    ...
    <div style={{minHeight:"100vh",background:"#060612",color:"#e2e8f0",maxWidth:480,margin:"0 auto",position:"relative",fontFamily:"'Rajdhani',sans-serif"}}>
      ...
      <div style={{padding:"18px 16px 90px",position:"relative",zIndex:1}}>
        ...
        {tab==="today"&&<>
          ...
          {allDone&&<div style={{background:"linear-gradient(135deg,rgba(56,189,248,.1),rgba(192,132,252,.1))",border:"1px solid rgba(56,189,248,.38)",borderRadius:14,padding:"14px 18px",marginBottom:18,textAlign:"center",boxShadow:"0 0 24px rgba(56,189,248,.1)",animation:"slideUp .4s ease"}}>
            ...
            <div style={{fontSize:11,color:"#475569",marginTop:3}}>Daily + Weekly abgeschlossen</div>
          </div>}
          ...
          {weeklyQ.length>0&&<>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <div style={{fontSize:9,color:"#c084fc",background:"rgba(192,132,252,.15)",border:"1px solid rgba(192,132,252,.3)",borderRadius:20,padding:"2px 8px",fontWeight:700,letterSpacing:.5}}>{weeklyDone}/{weeklyQ.length}</div>
                </div>
                <div style={{fontSize:11,color:"#2d3f55",marginTop:2}}>
                  Reset {(()=>{const t=new Date(),off=(t.getDay()+6)%7,mon=new Date(t);mon.setDate(t.getDate()-off+7);return mon.toLocaleDateString("de-DE",{weekday:"short",day:"numeric",month:"short"});})()}
                </div>
              </div>
            ...
            {weeklyQ.map(t=><QuestRow key={t.id} t={t} done={weeklyDoneIds.has(t.id)} onToggle={()=>weeklyDoneIds.has(t.id)?doUndo(t.id,true):doComplete(t)} bon={bon} isWeekly/>)}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <div style={{fontSize:9,color:"#38bdf8",background:"rgba(56,189,248,.15)",border:"1px solid rgba(56,189,248,.3)",borderRadius:20,padding:"2px 8px",fontWeight:700,letterSpacing:.5}}>{dailyDone}/{dailyQ.length}</div>
              </div>
          ...
          <div style={{background:"rgba(8,14,30,.92)",border:"1px solid #1a2840",borderRadius:18,padding:"18px 14px",marginBottom:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4}}>
              {monArr.map((d,i)=>{
                ...
                return(<div key={d} style={{aspectRatio:"1",borderRadius:8,background:xp>0?`rgba(56,189,248,${.07+intensity*.55})`:"rgba(255,255,255,.018)",border:`1px solid ${isT?"#38bdf8":xp>0?"rgba(56,189,248,.22)":"transparent"}`,boxShadow:isT?"0 0 12px #38bdf845":"none",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:2}}>
        ...
        <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,.88)",backdropFilter:"blur(12px)",display:"flex",alignItems:"flex-end"}} onClick={e=>{if(e.target===e.currentTarget)setShowAdd(false)}}>
          <div className="modal-sheet" style={{width:"100%",maxWidth:480,margin:"0 auto",background:"#080d1c",borderRadius:"24px 24px 0 0",border:"1px solid #1a2840",borderBottom:"none",padding:"24px 18px 48px"}}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <input value={newQ.name} onChange={e=>setNewQ(q=>({...q,name:e.target.value}))} placeholder="z.B. Joggen gehen" autoFocus style={{width:"100%",background:"#111929",border:"1px solid #1e2f48",borderRadius:11,padding:"13px 15px",color:"#e2e8f0",fontSize:16,fontFamily:"'Rajdhani',sans-serif",fontWeight:600}}/>
            </div>
            <div style={{marginBottom:16}}>
